...
SRC = $(CASE)/ap
SRCDIR_16 = $(SRC)/test_usound_engine
SRCDIR_32 =
...
MEM_CHECK_TOOL = $(PSP_REL)/tools/module_mem_check.exe
...
#指定链接时搜索的路径
LINCLUDE = -L$(OBJ_DIR) -L$(PSPLIBDIR) -L$(ENHANCEDIR) -L$(CASELIBDIR) -L$(PSP_REL)/include -L$(CASE)/inc

#指定自定义链接脚本的名称
LD_SCRIPT = uengine.xn ./../common/common_engine.xn
...
# ap builder
# **********************************************
AP_PRIO = $(AP_BACK_LOW_PRIO)
AP_STK_POS = $(AP_BACK_LOW_STK_POS)
AP_STK_LEN = $(AP_BACK_LOW_STK_SIZE)
...
all : creat_dir $(IMAGE_ELF)
	...
	$(MEM_CHECK_TOOL) $(IMAGE_ELF) ./../../cfg/back_ap_mem_rules.cfg
	$(AP_BUILDER) $(IMAGE_ELF) $(TARGET_AP) $(AP_PRIO) $(AP_STK_POS) $(AP_STK_LEN)
	cp -f $(TARGET_AP) $(OBJECT_BIN_PATH)	

creat_dir:
	mkdir -p $(OBJ_DIR)
#链接过程
$(IMAGE_ELF) : $(OBJ)
	sde-size -x $(OBJ_DIR)/*.o > size.txt
